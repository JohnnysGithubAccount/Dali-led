...
#define dali_drv "my_device"

// view using cat /sys/kernel/debug/gpio
#define GPIO_14       585    
#define GPIO_15       586   

#define DALI_OUT_PORT    GPIO_14  // Tx
#define DALI_IN_PORT     GPIO_15  //Rx

#define DALI_BIT_HALF_PERIOD 10000000 //10.000.000 nanoseconds -> 0.01 second
#define DALI_STOPBIT_VAL    3	// 0011 stop bit value (no phase change)
 
static dev_t my_device_number;         // Variable for the first device number
static struct cdev my_device; // Structure to keep track of the character device
static struct class *my_class;// Class structure pointer
 

//this is the linked list node definition
typedef struct manchesterBitValLinkedList_t {
	struct manchesterBitValLinkedList_t *pNext; //the next node
	uint8_t bitValue;
}manchesterBitValLinkedList_t;

static manchesterBitValList_t* pBitValRoot = NULL;  //initial the root node to be null

static struct hrtimer high_res_timer;  //timer used for timming bit sending action 
...
static ssize_t RPi_dali_read( struct file* F, char *buf, size_t count, loff_t *f_pos )
{
	...
	printk("The value got the from gpio is: %d", temp);
	
	sprintf( GPIOString_in, "%1d" , temp );
	count = sizeof( GPIOString_in ); //count of bytes to read
	// buffer to read data into from file F at position f_pos
	if (copy_to_user( buf, GPIOString_in, count )){
		return -EFAULT;
	}
...
static void dali_manchesterListAddVal(uint8_t value){
	manchesterBitValList_t* pTemp = kmalloc(sizeof(manchesterBitValList_t), GFP_KERNEL);
  
	//pointer to normally (GFP_KERNEL flag) allocated memory inside the kernel space memory
...
static ssize_t RPi_dali_write( struct file* F, const char *buf, size_t count, loff_t *f_pos )
{
	...
	printk("Initial variables successfully in the write function");
...
static int RPi_dali_open( struct inode *inode, struct file *file ){printk("Device opended"); return 0;}
 
static int RPi_dali_close( struct inode *inode, struct file *file ){printk("Device closed"); return 0;}
...
{ 
	...
	printk("Init GPIO14 successfully\n");
	...
	printk("Init GPIO15 successfully\n");
	...
	if ( (cl_pointer = class_create("RPi_Dali_drv" ) ) == NULL )
	{
		printk( KERN_ALERT "Class creation failed\n" );
		unregister_chrdev_region( first_dev, 1 );
		return -1;
	}
	printk("Class creation successfully");
	...
	printk("Device creation successfully");
	...
	printk("Device addition successfully");
	...
	printk("Nothing wrong in the initization of the driver");
	return 0;
}
