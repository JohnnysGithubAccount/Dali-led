...
#define DALI_BIT_HALF_PERIOD 10000000 //nanoseconds
...
static void dali_manchesterListAddVal(uint8_t val)
{
  ...
  //if(!pTemp->pNext){kfree(pTemp);return;}
...
static ssize_t RPi_dali_write( struct file* F, const char *buf, size_t count, loff_t *f_pos )
{
	...
	if (ret){printk("Failed to copy data from user"); return -EFAULT;};
