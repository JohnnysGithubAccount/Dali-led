    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        {
            ...
            if (ImGui::Button("Send command"))
            {
                ...
                if (inet_pton(AF_INET, "192.168.137.170", &serv_addr.sin_addr) <= 0)
                {
                ...
                {
                    snprintf(buffer, sizeof(buffer), "%03d%03d", unsignedAddressValue, unsignedDataValue); // Convert integer to string
